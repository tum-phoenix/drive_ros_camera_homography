...
public:
    ...
    : it_(nh_)
  {
    ...
    image_sub_ = it_.subscribe("/camera/image_raw", 1, &CameraHomography::imageCb, this);
    cam_info_sub = nh_.subscribe("/camera/camera_info", 1, &CameraHomography::camInfoCb, this);
    image_pub_ = it_.advertise("/image_converter/output_video", 1);
    homCalculated = false;
    patternSize_ = cv::Size(4, 15); // 4 columns, 15 rows
    length = 6;  // diameter circle
    xoff = 21.1; // x-offset of lower-left circle in centimeters
    yoff = 45.3; // y-offset of lower-left circle in centimeters

    // x-displacement of each circle: 150 mm
    // y-displacement of each circle: 100 mm
